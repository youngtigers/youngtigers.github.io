%%
%%  Lprime = loglik_K2_prime(omegabar, y, z, sigvhat, pi) 
%%
...
function Lprime = loglik_K2_prime(omegabar, y, z1, z2, sigvhat_g1, sigvhat_g2, pi_g1, pi_g2, K_sieves ) 
...
sqrt_s_v2 = sqrt(sigvhat_g2); % sqrt_s_v1 is the square root of sigma_v (in std); note sigvhat = sqrt(sigv2hat_g2)
s_v1 = sqrt_s_v1^2;   % square it to make it sigma_v1
s_v2 = sqrt_s_v2^2;   % square it to make it sigma_v1
...
%test
%L1 = sum(log((1-tau)*exp(-L11) + tau*exp(-L12)));
%L2 = sum(log((1-tau)*exp(-L21) + tau*exp(-L22)));
